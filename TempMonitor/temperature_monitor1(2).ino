...
void loop()
{
  ...
  if (celsius >= baselineTemp) {
    ...
    while(buttonState==LOW){
      blink();
      delay(200);
      //reverse countdown so that the user can push the button on time
      //nad the system can detect it
      Serial.print("Push the button in ");
      for(int i=10; i>0; i--){
        if(buttonState==HIGH)// if the button is pushed stop the countdown and deliver it to the system
          break;
        Serial.print(i);
        Serial.print(",");   
        delay(50);
